...
<Steps>
  <Step title="Create a session">
    POST `/api/v1/kyc/sessions` with your Bearer public API key. Response includes `sessionId` and `verificationUrl`.
  </Step>
  ...
  <Step title="Retrieve the session">
    GET `/api/v1/kyc/sessions/{sessionId}` to check ownership and status.
  </Step>
...
- Create session: `POST /api/v1/kyc/sessions`
