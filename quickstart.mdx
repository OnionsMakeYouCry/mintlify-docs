...
- **Create a session**: You call `POST /api/v1/kyc/sessions` with your Bearer public key. You receive a `sessionId` and a `verificationUrl`.
- **Send your user**: Redirect or link your user to the `verificationUrl` to complete the hosted verification flow.
- **Check status**: You can `GET /api/v1/kyc/sessions/{sessionId}` to verify ownership and see the current `status`.

## Authentication

Use your public API key in the Authorization header. Public keys map to users in Firestore at `users.apiPublicKey`.
